...
MARKET_PROOF = {
"no": ["Et av verdens mest tillitsbaserte markedsområder.", "Alle fem nordiske land er blant verdens ti høyest rangerte — både på livstilfredshet og lav opplevd offentlig korrupsjon.", "innbyggere", "Nordens største marked", "kompakt marked", "livstilfredshet · korrupsjon", "Rangeringer: World Happiness Report 2026 og Transparency International Corruption Perceptions Index 2025. Befolkning: 1. januar 2025. Indikatorene peker på stabile samfunn med høy tillit — ikke på at alle produkter automatisk har et marked."],
"en": ["One of the world’s highest-trust market regions.", "All five Nordic countries rank among the world’s top ten — both for life satisfaction and low perceived public-sector corruption.", "residents", "the Nordic region’s largest market", "compact market", "life satisfaction · corruption", "Rankings: World Happiness Report 2026 and Transparency International Corruption Perceptions Index 2025. Population: 1 January 2025. The indicators point to stable, high-trust societies — not to every product automatically having a market."],
"de": ["Eine der vertrauensstärksten Marktregionen der Welt.", "Alle fünf nordischen Länder zählen weltweit zu den zehn bestplatzierten — sowohl bei der Lebenszufriedenheit als auch bei geringer wahrgenommener Korruption im öffentlichen Sektor.", "Einwohner", "größter Markt der nordischen Region", "kompakter Markt", "Lebenszufriedenheit · Korruption", "Rankings: World Happiness Report 2026 und Transparency International Corruption Perceptions Index 2025. Bevölkerung: 1. Januar 2025. Die Indikatoren weisen auf stabile Gesellschaften mit hohem Vertrauen hin — nicht darauf, dass jedes Produkt automatisch einen Markt hat."],
}

def market_page(locale):
    t, m = LOCALES[locale], MARKET[locale]
    proof = MARKET_PROOF[locale]
    descriptors = [proof[2], proof[2], proof[3], proof[2], proof[4]]
    ranks = ["#3 · #1", "#6 · #4", "#5 · #6", "#1 · #2", "#2 · #10"]
    populations = {"no":["6,0 mill.","5,6 mill.","10,6 mill.","5,6 mill.","0,4 mill."],"en":["6.0m","5.6m","10.6m","5.6m","0.4m"],"de":["6,0 Mio.","5,6 Mio.","10,6 Mio.","5,6 Mio.","0,4 Mio."]}[locale]
    cards = "".join(f'<a class="country-card" href="{SLUGS[locale][k]}.html"><span class="country-flag" aria-hidden="true">{COUNTRIES[k]["flag"]}</span><h3>{COUNTRIES[k]["names"][locale]}</h3><span class="country-stat"><strong>{populations[i]}</strong>{descriptors[i]}</span><span class="country-stat"><strong>{ranks[i]}</strong>{proof[5]}</span><span class="country-link">{m[8]} →</span></a>' for i,k in enumerate(COUNTRIES))
