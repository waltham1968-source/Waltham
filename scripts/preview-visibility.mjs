...
http.createServer(async(req,res)=>{
 try{
  if(req.url==='/api/ai-visibility'){
   ...
   const result=await handler(new Request('http://localhost/api/ai-visibility',{method:req.method,headers:req.headers,body:['GET','HEAD'].includes(req.method)?undefined:body}));res.writeHead(result.status,Object.fromEntries(result.headers));res.end(await result.text());return;
  }
