...
(function () {
  ...
  // Compact navigation used by the newer case-journey pages.
  const compactToggle = document.querySelector('.menu');
  const compactLinks = document.querySelector('.links');
  if (compactToggle && compactLinks) {
    compactToggle.addEventListener('click', function () {
      const isOpen = compactLinks.classList.toggle('open');
      compactToggle.setAttribute('aria-expanded', String(isOpen));
    });

    compactLinks.querySelectorAll('a').forEach(function (link) {
      link.addEventListener('click', function () {
        compactLinks.classList.remove('open');
