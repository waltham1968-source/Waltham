...
form.addEventListener('submit',async event=>{
 ...
 try{
  ...
  const reviewNode=$('#google-reviews');reviewNode.textContent=data.google.reviewsCount===null?(data.google.reviewsStatus==='not_configured'?v2Copy.reviewsPending:v2Copy.unknown):`${data.google.reviewsCount} ${v2Copy.reviews}${data.google.reviewsRating===null?'':` · ${data.google.reviewsRating.toLocaleString(formats[locale])}/5`}`;
  ...
  if(data.google.reviewsCount===null){const link=document.createElement('a');link.href=`https://www.google.com/search?q=${encodeURIComponent(`${data.impression.identity||new URL(data.url).hostname} ${new URL(data.url).hostname} anmeldelser`)}`;link.target='_blank';link.rel='noopener noreferrer';link.textContent=v2Copy.searchReviews;reviewNode.append(' · ',link);}
